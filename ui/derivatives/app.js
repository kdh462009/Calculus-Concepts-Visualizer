...
function drawFunctionOnly() {
  ...
  el.analysis.textContent = "f(x) plotted. Press ANIMATE DERIVATIVE to start slope animation.";
...
function wireInteractions() {
  ...
  el.pauseBtn.addEventListener("click", () => {
    if (!anim.animating) {
      setStatus("nothing to pause. press ANIMATE DERIVATIVE first.");
      return;
    }
  ...
  el.resetBtn.addEventListener("click", () => {
    ...
    setStatus("reset.");
    setCounter("");
    el.analysis.textContent = "Pick a function, then press ANIMATE DERIVATIVE.";
    setConcavityText("Concavity insight appears when second derivative is enabled.");
  });
...
async function bootstrap() {
  ...
  functionPreview = FunctionPreview.wire({
    ...
    onPlotted: () => {
      drawFunctionOnly();
      setStatus("f(x) plotted. press ANIMATE DERIVATIVE.");
    },
